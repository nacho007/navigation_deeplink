...
fun TestBottomSheet(
    ...
    flagUrl: String,
    callback: (TestBottomSheet) -> Unit = { }
) {
    if (state.country == null) {
        callback.invoke(TestBottomSheet.LoadCountry)
    }
    ...
    Column(
        ...
        state.country?.let { country ->
            PhoneNumberTextField(
                ...
                onTextChanged = { callback.invoke(TestBottomSheet.OnChangePhone(it)) },
                changeCountry = {
                    callback.invoke(TestBottomSheet.LoadCountries)
                },
                onComplete = {}
            )
        }
    ...
    Scaffold(
        topBar = {
            SearchToolbar(placeholderText = "Search Country",
                ...
                onClearClick = { callback.invoke(TestBottomSheet.OnClearSearch) },
                onSearchTextChanged = { callback.invoke(TestBottomSheet.OnSearchCountry(it)) }
            ) {
                IconNavigationBack {
                    callback.invoke(TestBottomSheet.CloseCountryList)
                }
            }
        }
    ) {
    ...
    AstroCardView(
        ...
        Column {
            Row(
                ...
                modifier = Modifier
                    .fillMaxWidth()
                    .clickable { callback.invoke(TestBottomSheet.OnSelectCountry(country = country)) }
            ) {
...
sealed class TestBottomSheet {
    object LoadCountry : TestBottomSheet()
    object LoadCountries : TestBottomSheet()
    data class OnChangePhone(val phone: String) : TestBottomSheet()
    data class OnSelectCountry(val country: Country) : TestBottomSheet()
    object CloseCountryList : TestBottomSheet()
    data class OnSearchCountry(val text: String) : TestBottomSheet()
    object OnClearSearch : TestBottomSheet()
    object OnNextButton : TestBottomSheet()
}
